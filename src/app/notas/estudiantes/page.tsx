...
export default function EstudiantesPage() {
  ...
  // 💡 EL DICCIONARIO MAESTRO ACTUALIZADO CON TUS IDs REALES
  const NIVELES = [
    {
      nivel: "Pre-Primaria",
      grados: [
        { id: '11', nombre: 'Nursery I', secciones: [{ id: '4', label: 'Única' }] },
        { id: '12', nombre: 'Nursery II', secciones: [{ id: '4', label: 'Única' }] },
        { id: '13', nombre: 'Nursery III', secciones: [{ id: '4', label: 'Única' }] },
    ...
    },
    {
      nivel: "Primaria",
      grados: [
        { id: '6', nombre: 'Primero Primaria', secciones: [{ id: '1', label: 'A' }, { id: '2', label: 'B' }] },
        { id: '7', nombre: 'Segundo Primaria', secciones: [{ id: '1', label: 'A' }, { id: '2', label: 'B' }] },
        { id: '2', nombre: 'Tercero Primaria', secciones: [{ id: '1', label: 'A' }, { id: '2', label: 'B' }] },
        { id: '8', nombre: 'Cuarto Primaria', secciones: [{ id: '1', label: 'A' }, { id: '2', label: 'B' }] },
        { id: '9', nombre: 'Quinto Primaria', secciones: [{ id: '1', label: 'A' }, { id: '2', label: 'B' }] },
        { id: '10', nombre: 'Sexto Primaria', secciones: [{ id: '1', label: 'A' }, { id: '2', label: 'B' }] },
      ]
    },
    {
      nivel: "Básico",
    ...
    const cargarTodosLosConteos = async () => {
      ...
      for (const g of todosLosGrados) {
        ...
        try {
          ...
          // Actualizamos la pantalla con este número y pasamos al siguiente
  ...
  const handleInscribir = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      // Sube el número de alumnos del círculo en 1
      setConteos(prev => ({ 
        ...prev, 
        [gradoSeleccionado]: typeof prev[gradoSeleccionado] === 'number' ? (prev[gradoSeleccionado] as number) + 1 : 1 
      }));
    }
    setCargando(false);
  };
  ...
  const toggleGrado = (g: any) => {
    if (abierto === g.id) {
      ...
      setAbierto(g.id);
      // Abre la primera sección disponible por defecto
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      <header className="bg-red-700 text-white shadow-lg p-6 mb-10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <Link href="/notas" className="bg-white text-red-700 px-6 py-2 rounded-xl font-black text-xs uppercase shadow-md hover:bg-red-50 transition-colors">
            Volver al Menú
          </Link>
        </div>
      </header>
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={e => { 
                      ...
                      const obj = NIVELES.flatMap(n => n.grados).find(g => g.id === e.target.value);
                      if(obj && obj.secciones.length > 0) setSeccionSeleccionada(obj.secciones[0].id);
                    }} 
                ...
                <div>
                  ...
                  >
                    ...
                    {gradoSeleccionadoObj?.secciones.map((sec: any) => (
                      <option key={sec.id} value={sec.id}>{sec.label === 'Única' ? 'Única' : `Sec. ${sec.label}`}</option>
                    ))}
                  </select>
                </div>
              </div>
                  ...
                  <div key={g.id} className={`bg-white rounded-[2rem] border transition-all ${abierto === g.id ? 'shadow-xl border-red-100' : 'shadow-sm border-gray-100'}`}>
                    ...
                    <button onClick={() => toggleGrado(g)} className="w-full p-6 flex justify-between items-center group">
                      <div className="flex items-center space-x-5">
                        ...
                        {/* 💡 AQUÍ MOSTRAMOS EL CONTEO EN EL CÍRCULO */}
                        <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center transition-all ${abierto === g.id ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 group-hover:bg-red-50 group-hover:text-red-600'}`}>
                          {conteos[g.id] !== undefined ? (
                            <>
                              <span className="font-black text-lg leading-none">{conteos[g.id]}</span>
                              <span className="text-[7px] font-bold uppercase tracking-widest opacity-80 mt-0.5">Alumnos</span>
                            </>
                          ) : (
                        ...
                        <div className="text-left">
                          ...
                          <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                            {g.secciones.length > 1 ? `${g.secciones.length} Secciones` : 'Sección Única'}
                          </span>
                        </div>
                      </div>
                      <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-bold transition-colors ${abierto === g.id ? 'border-red-600 text-red-600 bg-red-50' : 'border-gray-200 text-gray-400'}`}>
                        {abierto === g.id ? '−' : '+'}
                      </div>
                    </button>
                        ...
                        <div className="flex space-x-2 mb-6 p-1 bg-gray-100 rounded-xl w-fit">
                          {g.secciones.length > 1 ? (
                            g.secciones.map((sec: any) => (
                              <button key={sec.id} onClick={() => cambiarSeccionVista(sec.id)} className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${seccionVisualizacion === sec.id ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}>
                                Sección {sec.label}
                              </button>
                            ))
                          ) : (
