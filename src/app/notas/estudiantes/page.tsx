...
} from "../../actions"; // 💡 IMPORTAMOS LOS PERMISOS
...
export default function EstudiantesPage() {
  ...
  // 💡 ESTADOS PARA LA EDICIÓN (Pegar aquí)
  const [editandoEstudiante, setEditandoEstudiante] = useState<any>(null);
  const [nombreEdit, setNombreEdit] = useState("");
  const [apellidoEdit, setApellidoEdit] = useState("");
  const [gradoEdit, setGradoEdit] = useState("");
  const [seccionEdit, setSeccionEdit] = useState("");
  ...
  const abrirEdicion = (est: any, gradoId: string, secId: string) => {
    // Intentamos separar el nombre del apellido para los inputs
    const partes = est.nombre.split(" ");
    const primerNombre = partes[0] || "";
    const elResto = partes.slice(1).join(" ") || "";

    setEditandoEstudiante(est);
    setNombreEdit(primerNombre);
    setApellidoEdit(elResto);
    setGradoEdit(gradoId);
    setSeccionEdit(secId);
  };

  const handleActualizar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nombreEdit || !apellidoEdit)
      return alert("⚠️ No dejes campos vacíos.");
    setCargando(true);

    const res = await actualizarEstudiante(
      Number(editandoEstudiante.codigo),
      nombreEdit,
      apellidoEdit,
      Number(gradoEdit),
      Number(seccionEdit),
    ...
    if (res.success) {
      alert("✅ Alumno actualizado en Staging");
      setEditandoEstudiante(null);
      cargarEstudiantesGrado(gradoEdit, seccionEdit); // Refresca la lista
    } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="bg-white p-10 rounded-3xl shadow-lg text-center border-2 border-red-100">
          ...
          <h1 className="text-2xl font-black text-[#E60000] mb-2 uppercase">
            Acceso Denegado
          </h1>
          ...
          <Link
            href="/notas"
            className="px-6 py-3 bg-[#17365D] text-white rounded-xl font-black text-xs uppercase hover:bg-slate-800 transition-colors"
          >
            Volver al Menú
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      <header className="bg-[#E60000] text-white shadow-lg p-6 mb-10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <Link
            href="/notas"
            className="bg-white text-[#E60000] px-6 py-2 rounded-xl font-black text-xs uppercase shadow-md hover:bg-red-50 transition-colors"
          >
            Volver al Menú
          </Link>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 sticky top-10">
            <h2 className="text-xl font-black text-[#17365D] uppercase mb-6 italic">
              Inscribir Alumno
            </h2>
            <form onSubmit={handleInscribir} className="space-y-4">
                  ...
                  >
                    ...
                    {gradoSeleccionadoObj?.secciones.map((sec: any) => (
                      <option key={sec.id} value={sec.id}>
                        {sec.label === "Única" ? "Única" : `Sec. ${sec.label}`}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <button
                disabled={cargando}
                className="w-full py-5 bg-[#E60000] text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg hover:bg-red-700 transition-all active:scale-95 disabled:opacity-50 mt-4"
              >
          ...
          {NIVELES.map((nivel) => (
            <div key={nivel.nivel} className="relative">
              <div className="flex items-center mb-6">
                <h2 className="text-xl font-black text-[#E60000] uppercase tracking-tighter mr-4 bg-slate-50 relative z-10 pr-4">
                  {nivel.nivel}
                </h2>
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center space-x-5">
                        <div
                          className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center transition-all ${abierto === g.id ? "bg-[#E60000] text-white shadow-lg" : "bg-gray-100 text-gray-600 group-hover:bg-red-50 group-hover:text-red-600"}`}
                        >
                      ...
                      <div
                        className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-bold transition-colors ${abierto === g.id ? "border-[#E60000] text-red-600 bg-red-50" : "border-gray-200 text-gray-400"}`}
                      >
                        {abierto === g.id ? "−" : "+"}
                      </div>
                    </button>
                          ...
                          {g.secciones.length > 1 ? (
                            g.secciones.map((sec: any) => (
                              <button
                                ...
                                className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${seccionVisualizacion === sec.id ? "bg-white text-[#E60000] shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
                              >
                                Sección {sec.label}
                              </button>
                            ))
                            ...
                            <tbody className="divide-y divide-gray-100">
                              {gradosVisibles[`${g.id}-${seccionVisualizacion}`]
                                ...
                                gradosVisibles[
                                  ...
                                  >
                                    <td className="px-8 py-5">
                                      <span className="bg-white px-3 py-1.5 rounded-lg text-[11px] text-[#17365D] font-black border border-blue-100 shadow-sm">
                                        {est.codigo}
                                      </span>
                                    </td>
                                    ...
                                    {/* 👈 PEGA ESTO AQUÍ: BOTÓN DE EDICIÓN */}
                                ...
                                <tr>
                                  ...
                                  >
                                    Sin alumnos registrados en esta sección
                                  </td>
                                </tr>
                              )}
                            </tbody>
      ...
      {/* 👈 PEGA ESTO AQUÍ: MODAL DE EDICIÓN FLOTANTE */}
      {editandoEstudiante && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] p-10 max-w-md w-full shadow-2xl border-4 border-[#17365D] animate-in fade-in zoom-in duration-300">
            <h2 className="text-2xl font-black text-[#17365D] uppercase mb-6 italic">Editar Estudiante</h2>
            
            <form onSubmit={handleActualizar} className="space-y-5">
              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-2">Nombres</label>
                <input 
                  value={nombreEdit} 
                  onChange={(e) => setNombreEdit(e.target.value)} 
                  className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl font-bold uppercase outline-none focus:border-blue-500 transition-colors" 
                />
              </div>
              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-2">Apellidos</label>
                <input 
                  value={apellidoEdit} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase mb-2">Grado</label>
                  <select 
                    value={gradoEdit} 
                    onChange={(e) => setGradoEdit(e.target.value)} 
                    className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl font-bold text-[10px] uppercase outline-none focus:border-blue-500"
                  >
                    {NIVELES.flatMap(n => n.grados).map(g => (
                      <option key={g.id} value={g.id}>{g.nombre}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase mb-2">Sección</label>
                  <select 
                    value={seccionEdit} 
                    onChange={(e) => setSeccionEdit(e.target.value)} 
                    className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl font-bold text-sm outline-none focus:border-blue-500"
                  >
              ...
              <div className="flex space-x-4 pt-4">
                ...
                <button 
                  type="submit" 
                  disabled={cargando} 
                  className="flex-1 py-4 bg-[#17365D] text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg active:scale-95 disabled:opacity-50 transition-all"
                >
